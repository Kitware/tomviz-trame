...
from tomviz._internal import in_application, require_internal_mode
...
def zoom_shape(input: np.ndarray, zoom: np.ndarray) -> tuple[int]:
    ...
    return tuple([int(round(i * j)) for i, j in zip(input.shape, zoom, strict=False)])


def rotate_shape(input: np.ndarray, angle: float, axes: tuple[int, int]) -> tuple[int]:
    """
    ...
    mtrx = np.array([[m11, -m21], [-m12, m22]], dtype=np.float64)
    minc = [0, 0]
    maxc = [0, 0]
...
def apply_to_each_array(func):
    ...
    """
    is_method = (
        func.__name__ != func.__qualname__ and ".<locals>." not in func.__qualname__
    )
    ...
    def wrapper(*args, **kwargs):
        ...
        for i, name in enumerate(array_names):
            ...
            new_args = (
                list(args[:dataset_idx]) + [dataset] + list(args[dataset_idx + 1 :])
            )
        ...
        if is_internal:
            ...
            dataset.arrays.clear()
            for name, array in zip(array_names, output_arrays, strict=False):
            ...
            for k, v in result.items():
                if isinstance(v, Dataset):
                    ...
                    for i, other_result in enumerate(results[:-1]):
                        if isinstance(other_result, dict) and isinstance(
                            other_result.get(k), Dataset
                        ):
                            other_dataset = other_result[k]
                            other_dataset.rename_active(array_names[i])
                            v.set_scalars(
                                other_dataset.active_name, other_dataset.active_scalars
                            )
...
def make_spreadsheet(column_names: list[str], table: np.ndarray) -> "vtkTable":
    ...
    from vtk import vtkFloatArray, vtkTable
