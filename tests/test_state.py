...
import pytest
from tomviz_pipeline import SinkNode
...
def test_collapse_sink_groups_relinks_sinks_to_the_upstream_port():
    pipeline = pipeline_from_state_dict(STATE)
    reader = pipeline.node_by_id(1)
    assert state.collapse_sink_groups(pipeline) == 1

    assert pipeline.node_by_id(2) is None
    for sink_id in (3, 4):
        sink = pipeline.node_by_id(sink_id)
        assert isinstance(sink, SinkNode)
        assert state.primary_upstream(sink) is reader.output_port("volume")
    assert len(pipeline.links) == 2
    assert state.collapse_sink_groups(pipeline) == 0
